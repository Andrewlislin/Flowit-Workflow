...
async function runDaemon(args: string[]): Promise<void> {
  ...
  const rawAdapters = (option(args, 'adapters') ?? process.env.FLOWIT_WORKFLOW_ADAPTERS)
    ?.split(',')
    .map(value => value.trim())
    .filter(Boolean)
  const adapters: BuiltInAdapterId[] =
    rawAdapters?.map(value => requireBuiltInAdapterId(value, 'adapters')) ?? [adapter]
  const instanceId = option(args, 'instance') ?? process.env.FLOWIT_WORKFLOW_INSTANCE_ID
  const resolved = resolveConfiguredRuntime({
    defaultAdapterId: adapter,
    adapterIds: adapters,
  ...
  if (args.includes('--detach')) {
    ...
    const child = spawn(process.execPath, [process.argv[1]!, 'daemon', ...clean], {
      detached: true,
      stdio: 'ignore',
  ...
  try {
    ...
    console.error(
      `Flowit Workflow daemon ready for ${adapters.join(',')} (instance ${resolved.instanceId}, pid ${process.pid})`,
    )
