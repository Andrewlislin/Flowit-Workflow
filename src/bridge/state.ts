...
export interface BridgeStatePaths { root: string; sessionsFile: string; eventsFile: string; cursorFile: string; inboxDir: string; outboxDir: string }
...
export function bridgeStatePaths(adapterId: string, root?: string): BridgeStatePaths {
  ...
  return { root: base, sessionsFile: path.join(base, 'sessions.json'), eventsFile: path.join(base, 'events.jsonl'), cursorFile: path.join(base, 'events.cursor'), inboxDir: path.join(base, 'inbox'), outboxDir: path.join(base, 'outbox') }
}
...
export async function upsertBridgeSession(paths: BridgeStatePaths, session: BridgeSessionRecord): Promise<void> {
  await withFileLock(paths.sessionsFile, async () => {
...
export async function readBridgeEventsAfter(paths: BridgeStatePaths, offset: number): Promise<{ events: AgentEvent[]; nextOffset: number }> {
  let text = ''
  try { text = await readFile(paths.eventsFile, 'utf8') }
...
export async function readBridgeCursor(paths: BridgeStatePaths): Promise<number> {
  try { const value = Number((await readFile(paths.cursorFile, 'utf8')).trim()); return Number.isSafeInteger(value) && value >= 0 ? value : 0 }
...
async function withFileLock<T>(file: string, operation: () => Promise<T>): Promise<T> {
  ...
  while (true) {
    try {
      ...
      finally { await handle.close().catch(() => undefined); await rm(lock, { force: true }).catch(() => undefined) }
    } catch (error: unknown) {
      if ((error as NodeJS.ErrnoException).code !== 'EEXIST') throw error
      try { if (Date.now() - (await stat(lock)).mtimeMs > 60_000) await rm(lock, { force: true }) } catch {}
      if (Date.now() >= deadline) throw new Error(`timed out waiting for bridge state lock: ${lock}`)
...
function isSession(value: unknown): value is BridgeSessionRecord {
  ...
  return typeof row.adapterId === 'string' && typeof row.sessionId === 'string' && typeof row.status === 'string'
}
