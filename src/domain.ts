...
export function normalizeContextRefs(refs: readonly SessionContextRef[] | undefined): SessionContextRef[] {
  ...
  for (const ref of refs ?? []) {
    ...
    result.push(label ? { sessionId, label } : { sessionId })
  }
  return result
}

export function normalizeTarget(target: AutomationTarget): AutomationTarget {
  return {
    ...
    contextRefs: normalizeContextRefs(target.contextRefs),
  }
}
...
export function createScheduleRecord(
  ...
  if (input.timing.kind === 'every') {
    if (!Number.isSafeInteger(input.timing.everySeconds) || input.timing.everySeconds < minimumIntervalSeconds) {
  ...
  const timing = input.timing.kind === 'at'
    ? { kind: 'at' as const, at: new Date(input.timing.at).toISOString() }
    : { kind: 'every' as const, everySeconds: input.timing.everySeconds }
  const nextRunAt = timing.kind === 'at'
    ? timing.at
    : new Date(now.getTime() + timing.everySeconds * 1000).toISOString()
  return {
    id,
...
export function advanceSchedule(
  task: ScheduledTask,
  firedAt: Date,
  oneShotOutcome: 'completed' | 'failed' = 'completed',
): ScheduledTask {
...
export function normalizePipeline(id: string, input: CreatePipelineInput, now: Date): PipelineDefinition {
  ...
  const nodes: PipelineNode[] = input.nodes.map(node => {
    ...
    return {
      ...
      inheritUpstreamContext: node.inheritUpstreamContext !== false,
    }
  })
  const edges: PipelineEdge[] = input.edges.map(edge => {
    const from = nonEmpty(edge.from, 'edge.from')
    const to = nonEmpty(edge.to, 'edge.to')
  ...
  const trigger = input.trigger.kind === 'manual'
    ...
    : { kind: 'session_turn_completed' as const, sessionId: nonEmpty(input.trigger.sessionId, 'trigger.sessionId') }
...
export function topologicalOrder(nodes: readonly PipelineNode[], edges: readonly PipelineEdge[]): string[] {
  ...
  for (const edge of edges) {
    if (!indegree.has(edge.from) || !indegree.has(edge.to)) throw new Error(`unknown pipeline edge: ${edge.from} -> ${edge.to}`)
...
export function predecessorIds(nodeId: string, edges: readonly PipelineEdge[]): string[] {
  return edges.filter(edge => edge.to === nodeId).map(edge => edge.from)
}

export function assertNoAutonomousSessionCycle(pipelines: readonly PipelineDefinition[]): void {
  ...
  for (const pipeline of pipelines) {
    if (pipeline.status !== 'active' || pipeline.trigger.kind !== 'session_turn_completed') continue
    const nodes = new Map(pipeline.nodes.map(node => [node.id, node]))
    const roots = pipeline.nodes.filter(node => !pipeline.edges.some(edge => edge.to === node.id))
    for (const root of roots) ensure(pipeline.trigger.sessionId).add(root.target.sessionId)
    for (const edge of pipeline.edges) {
      const from = nodes.get(edge.from)
      const to = nodes.get(edge.to)
      if (from && to) ensure(from.target.sessionId).add(to.target.sessionId)
    }
  }
  const visiting = new Set<string>()
  const visited = new Set<string>()
  const visit = (id: string): void => {
    if (visiting.has(id)) throw new Error(`autonomous session trigger cycle detected at ${id}`)
