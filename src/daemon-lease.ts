...
export async function acquireDaemonLease(instanceId: string, storageFile: string, options: AcquireDaemonLeaseOptions = {}): Promise<DaemonLease> {
  ...
  while (Date.now() < deadline) {
    ...
    try {
      await mkdir(lockDir)
      const metadata: DaemonLeaseMetadata = { version: 1, ownerToken, pid: process.pid, instanceId, storageFile: canonicalStorageFile, startedAt: new Date().toISOString() }
...
async function readOwner(lockDir: string): Promise<DaemonLeaseMetadata | undefined> {
  try {
    const value = JSON.parse(await readFile(path.join(lockDir, 'owner.json'), 'utf8')) as DaemonLeaseMetadata
    if (value.version !== 1 || typeof value.ownerToken !== 'string' || !Number.isSafeInteger(value.pid) || typeof value.storageFile !== 'string') return undefined
    return value
  } catch (error: unknown) {
...
async function moveAside(lockDir: string, reason: string): Promise<boolean> {
  ...
  try { await rename(lockDir, stale) }
  catch (error: unknown) { if ((error as NodeJS.ErrnoException).code === 'ENOENT') return false; throw error }
