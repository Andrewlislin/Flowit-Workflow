...
export function resolveConfiguredRuntime(
  ...
  const instanceId = normalizeInstanceId(
    options.instanceId ?? process.env.FLOWIT_WORKFLOW_INSTANCE_ID ?? 'default',
  )
  ...
  const legacyStorageFiles =
    explicitLegacy ??
    (!explicitStorage && instanceId === 'default'
      ? BUILT_IN_ADAPTER_IDS.map(legacyDefaultStoragePath)
      : [])
  return {
    defaultAdapterId,
    adapterIds,
    instanceId,
    storageFile,
    legacyStorageFiles: [...new Set(legacyStorageFiles.map(value => path.resolve(value)))],
...
export function createConfiguredRuntime(
  ...
  const resolved = resolveConfiguredRuntime(options)
  const adapters = resolved.adapterIds.map(id => createBuiltInAdapter(id, resolved.storageFile))
  return new FlowitOrchestrationCore(
    {
      ...
      minimumIntervalSeconds: resolved.minimumIntervalSeconds,
      maxRunHistory: resolved.maxRunHistory,
      maxTerminalReceipts: resolved.maxTerminalReceipts,
      terminalReceiptRetentionMs: resolved.terminalReceiptRetentionMs,
      maxEventInbox: resolved.maxEventInbox,
      activeWorkers: resolved.activeWorkers,
      leaseDurationMs: resolved.leaseDurationMs,
      retryDelayMs: resolved.retryDelayMs,
      maxPipelineAttempts: resolved.maxPipelineAttempts,
      maxScheduleAttempts: resolved.maxScheduleAttempts,
    },
    adapters,
  )
}

export function createBuiltInAdapter(id: BuiltInAdapterId, consumerId?: string): AgentAdapter {
  switch (id) {
    case CLAUDE_CODE_ADAPTER_ID:
      return new ClaudeCodeAgentAdapter({
        ...
        ...(process.env.FLOWIT_WORKFLOW_PLUGIN_ROOT
          ...
          : {}),
        ...(process.env.FLOWIT_WORKFLOW_CLAUDE_ALLOW_LIVE_RESUME === '1'
          ? { allowResumeLiveSession: true }
          : {}),
      })
    case OPENCODE_ADAPTER_ID: {
      const baseUrl = process.env.FLOWIT_WORKFLOW_OPENCODE_URL
      if (!baseUrl)
    ...
    case DOUBAO_OFFICE_ADAPTER_ID:
      return new DoubaoOfficeAgentAdapter(consumerId ? { consumerId } : {})
    default:
      throw new Error(`unsupported built-in adapter: ${String(id)}`)
  }
}

export function defaultStoragePath(instanceId: string): string {
  return path.join(
    os.homedir(),
    '.flowit-workflow',
    'instances',
...
export function legacyDefaultStoragePath(adapterId: string): string {
  return path.join(os.homedir(), '.flowit-workflow', adapterId, 'workflow.json')
}
...
export function requireBuiltInAdapterId(value: unknown, field = 'adapter'): BuiltInAdapterId {
  if (!isBuiltInAdapterId(value))
    throw new Error(`${field} must be one of: ${BUILT_IN_ADAPTER_IDS.join(', ')}`)
  return value
}
function envAdapter(): BuiltInAdapterId | undefined {
  const value = process.env.FLOWIT_WORKFLOW_ADAPTER
  if (!value?.trim()) return undefined
  return requireBuiltInAdapterId(value, 'FLOWIT_WORKFLOW_ADAPTER')
}
...
function normalizeInstanceId(value: string): string {
  ...
  if (!normalized) throw new Error('orchestration instanceId must be non-empty')
  if (!/^[A-Za-z0-9._-]+$/.test(normalized))
    throw new Error(
      'orchestration instanceId may contain only letters, numbers, dot, underscore, and hyphen',
    )
  return normalized
}
