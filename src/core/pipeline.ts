...
export class PipelineRuntime {
  ...
  private attachAdapter(adapter: AgentAdapter): void { if (!adapter.subscribe || this.eventStops.has(adapter.id)) return; const stop = adapter.subscribe(event => this.onAgentEvent(event)); this.eventStops.set(adapter.id, stop) }
