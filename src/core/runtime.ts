...
export class FlowitOrchestrationCore {
  ...
  constructor(config: FlowitCoreConfig, initialAdapters: readonly AgentAdapter[] = []) {
    ...
    const minimumIntervalSeconds = positiveInteger(config.minimumIntervalSeconds ?? 60, 'minimumIntervalSeconds')
    const maxRunHistory = positiveInteger(config.maxRunHistory ?? 500, 'maxRunHistory')
    const maxTerminalReceipts = positiveInteger(config.maxTerminalReceipts ?? 100_000, 'maxTerminalReceipts')
    ...
    const leaseDurationMs = integerAtLeast(config.leaseDurationMs ?? 30_000, 1_000, 'leaseDurationMs')
    const retryDelayMs = positiveInteger(config.retryDelayMs ?? 5_000, 'retryDelayMs')
    const maxPipelineAttempts = positiveInteger(config.maxPipelineAttempts ?? 3, 'maxPipelineAttempts')
    const maxScheduleAttempts = positiveInteger(config.maxScheduleAttempts ?? 3, 'maxScheduleAttempts')
    ...
    this.dispatcher = new OrchestrationDispatcher(this.adapters, this.contextGraph, this.skillBinder, config.defaultAdapterId)
    const activeWorkers = config.activeWorkers ?? true
    this.scheduler = new DurableScheduler(this.store, this.dispatcher, minimumIntervalSeconds, config.defaultAdapterId, { workerId: this.workerId, leaseDurationMs, retryDelayMs, maxAttempts: maxScheduleAttempts }, activeWorkers)
    this.pipelines = new PipelineRuntime(this.adapters, this.store, this.dispatcher, this.contextGraph, config.defaultAdapterId, { workerId: this.workerId, leaseDurationMs, retryDelayMs, maxAttempts: maxPipelineAttempts })
...
function positiveInteger(value: number, name: string): number { if (!Number.isSafeInteger(value) || value < 1) throw new Error(`${name} must be a positive integer`); return value }
function integerAtLeast(value: number, minimum: number, name: string): number { if (!Number.isSafeInteger(value) || value < minimum) throw new Error(`${name} must be an integer >= ${minimum}`); return value }
async function settleWithin(promise: Promise<unknown>, timeoutMs: number): Promise<void> { await Promise.race([promise.then(() => undefined, () => undefined), new Promise<void>(resolve => { const timer = setTimeout(resolve, timeoutMs); timer.unref?.() })]) }
