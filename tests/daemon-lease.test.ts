...
function runContender(
  ...
  return new Promise((resolve, reject) => {
    const child = spawn(
      process.execPath,
      ['--import', 'tsx', fixture, storage, leaseRoot, gate, instanceId, '1200'],
...
test('different instance ids cannot own the same canonical storage file concurrently', async () => {
  ...
  try {
    ...
    await import('node:fs/promises').then(fs =>
      fs.symlink(storageDir, aliasDir, process.platform === 'win32' ? 'junction' : 'dir'),
    )
    ...
    const parsed = results.map(
      result => JSON.parse(result.stdout.trim()) as { acquired: boolean; error?: string },
    )
    assert.equal(
      parsed.filter(row => row.acquired).length,
      1,
...
test('forced child death leaves a stale lease that a new owner can recover immediately', async () => {
  ...
  const child = spawn(
    process.execPath,
    ['--import', 'tsx', fixture, storage, leaseRoot, gate, 'old-owner', '10000'],
    { stdio: ['ignore', 'pipe', 'pipe'], env: process.env },
  )
  child.stdout.setEncoding('utf8')
  try {
    ...
    const replacement = await acquireDaemonLease('replacement', storage, {
      root: leaseRoot,
      acquisitionTimeoutMs: 1_000,
    })
    try {
      assert.equal(replacement.storageFile, await canonicalStorageIdentity(storage))
    } finally {
...
test('lease release cannot delete a lock after owner token changes', async () => {
  ...
  try {
    ...
    await writeFile(
      ownerFile,
      `${JSON.stringify({ ...current, ownerToken: 'replacement-owner' }, null, 2)}\n`,
      'utf8',
    )
...
test('two contenders cannot both take over the same stale daemon generation', async () => {
  ...
  try {
    ...
    await writeFile(
      path.join(lockDir, 'owner.json'),
      `${JSON.stringify({ version: 1, ownerToken: 'dead-generation', pid: 2_147_483_647, instanceId: 'dead', storageFile: canonical, startedAt: new Date().toISOString() })}\n`,
      'utf8',
    )
