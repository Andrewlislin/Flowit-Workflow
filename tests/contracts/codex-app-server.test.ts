...
async function fakeCodex(root: string, mode: 'complete' | 'failed' | 'interrupted' | 'approval' | 'exit' | 'timeout', marker?: string): Promise<string> {
...
function request() { return { correlationId: 'idem-1', sessionId: 'thr-1', prompt: 'do work', skills: [], contextRefs: [] } }

test('completion delivered in same stdout batch is buffered instead of lost', async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), 'flowit-codex-complete-')); const adapter = new CodexAgentAdapter({ executable: await fakeCodex(root, 'complete'), requestTimeoutMs: 500, turnTimeoutMs: 500 })
  try { const result = await adapter.dispatch(request()); assert.equal(result.runId, 'turn-1') } finally { await adapter.dispose(); await rm(root, { recursive: true, force: true }) }
})

test('failed and interrupted turns reject instead of being recorded successful', async () => {
  for (const mode of ['failed','interrupted'] as const) { const root = await mkdtemp(path.join(os.tmpdir(), `flowit-codex-${mode}-`)); const adapter = new CodexAgentAdapter({ executable: await fakeCodex(root, mode), requestTimeoutMs: 500, turnTimeoutMs: 500 }); try { await assert.rejects(adapter.dispatch(request()), new RegExp(mode)) } finally { await adapter.dispose(); await rm(root, { recursive: true, force: true }) } }
})

test('string-id unattended approval requests are answered fail-closed', async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), 'flowit-codex-approval-')); const adapter = new CodexAgentAdapter({ executable: await fakeCodex(root, 'approval'), requestTimeoutMs: 500, turnTimeoutMs: 500 })
  try { const result = await adapter.dispatch(request()); assert.equal(result.runId, 'turn-1') } finally { await adapter.dispose(); await rm(root, { recursive: true, force: true }) }
})

test('child exit rejects waiters and turn timeout sends turn/interrupt', async () => {
  const exitRoot = await mkdtemp(path.join(os.tmpdir(), 'flowit-codex-exit-')); const exitAdapter = new CodexAgentAdapter({ executable: await fakeCodex(exitRoot, 'exit'), requestTimeoutMs: 500, turnTimeoutMs: 1000 })
  try { await assert.rejects(exitAdapter.dispatch(request()), /exited/) } finally { await exitAdapter.dispose(); await rm(exitRoot, { recursive: true, force: true }) }

  const timeoutRoot = await mkdtemp(path.join(os.tmpdir(), 'flowit-codex-timeout-')); const marker = path.join(timeoutRoot, 'interrupt.txt'); const timeoutAdapter = new CodexAgentAdapter({ executable: await fakeCodex(timeoutRoot, 'timeout', marker), requestTimeoutMs: 500, turnTimeoutMs: 50 })
  try { await assert.rejects(timeoutAdapter.dispatch(request()), /timed out/); for (let i=0;i<20;i++) { try { assert.equal(await readFile(marker,'utf8'),'interrupt'); return } catch { await new Promise(resolve => setTimeout(resolve,10)) } } assert.fail('turn/interrupt marker was not written') } finally { await timeoutAdapter.dispose(); await rm(timeoutRoot, { recursive: true, force: true }) }
})

test('adapter start rejects when the Codex executable cannot be spawned', async () => {
  const adapter = new CodexAgentAdapter({ executable: path.join(os.tmpdir(), `missing-codex-${Date.now()}`), requestTimeoutMs: 200 })
  try { await assert.rejects(adapter.start(), /ENOENT|exited|spawn/i) } finally { await adapter.dispose() }
})
