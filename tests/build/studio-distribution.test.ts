...
test('distribution includes the three Community Studio package manifests', async () => {
  for (const id of ['content-studio', 'research-lab', 'agent-team']) {
    const manifest = JSON.parse(
      await readFile(`studios/community/${id}/flowit.package.json`, 'utf8'),
    ) as { entryPreset?: string }
    assert.equal(manifest.entryPreset, id)
  }
})
